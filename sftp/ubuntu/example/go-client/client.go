...
var (
	// example input
	hostInput       = "localhost"   // host of ssh server
	portInput       = "3022"        // port of sftp server
	userInput       = "root"        // name of user on the ssh server
	passInput       = "custompwd"   // password for the user on the ssh server
	pathInput       = "/files"      // path of sftp folder
	privateKeyInput = "id_rsa"      // path to client private key
...
func main() {
	...
	key, err := ioutil.ReadFile(filepath.Join(os.Getenv("HOME"), ".ssh", privateKeyInput))
	if err != nil {
	...
	// leave your mark
	f, err := client.Create(filepath.Join(pathInput, "hello.txt"))
	if err != nil {
		log.Fatal(err)
	}
	...
	// open files in dir
	fis, err := client.ReadDir("/files")
	if err != nil {
		log.Fatal("error open directory: ", err)
	}
	// log.Println("dir content:")
	// log.Println(fis)
	for _, c := range fis {
		fmt.Println(c.Name())
	}
